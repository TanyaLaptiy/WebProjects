...
fromBuf.addEventListener("click", function () {
    ...
    for (let i = 0; i < arr.length; i++) {
        // console.log(arr[i]);
        // if(!arr[i].includes(" ")){
        //     alert("Запись партии разобрать не удалось.\nПожалуйста придерживайтесь формату:\n\n каждый ход должен иметь вид: ход белых ход черных\nОшибка была допущена здесь: " + arr[i]);
        //     reset(); return;
        // }
    ...
    for (let i = 0; i < resArr.length; i++) {
        ...
        if (!resArr[i].includes(":")) {
            alert("Запись партии разобрать не удалось.\nПожалуйста придерживайтесь формату:\n\n каждый ход должен иметь вид: [прежняя локация]:[новая локация]\nОшибка была допущена здесь: " + resArr[i]);
            reset(); return;
        }
    ...
    for (let i = 0; i < resArr.length; i++) {
        if ((!values.includes(resArr[i].split(":")[0])) || (!values.includes(resArr[i].split(":")[1]))) {
            alert("Восемь горизонталей доски обозначаются цифрами от 1 до 8, восемь вертикалей - малыми латинскими буквами от 'a' до 'h' (a, b, c, d, e, f, g, h)\nОшибка была допущена здесь: " + resArr[i]);
            reset(); return;
        }
        ...
        if (auto_checkAvailableJumpSpaces(end - start)) {
            ...
            alert("Запись партии разобрать не удалось.\nПожалуйста перепроверьте корректность всех ходов:\n\n \nОшибка была допущена здесь: " + resArr[i]);
            reset(); return;
        }
        // makeMove(end - start);
